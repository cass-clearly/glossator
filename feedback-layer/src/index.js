  ...
  for (const ann of comments) {
    ...
    try {
      ...
      if (range && ann.status !== 'closed') {
        const marks = highlightRange(range, ann.id);
        for (const mark of marks) {
          if (mark) attachPreviewListeners(mark, ann.id);
        }
        anchored.add(ann.id);
    ...
    try {
      ...
      if (range) {
        ...
        if (comment.status !== 'closed') {
          ...
          for (const mark of marks) {
            if (mark) attachPreviewListeners(mark, comment.id);
          }
        }
...
async function handleCommentSubmit({ comment, commenter }) {
  ...
  try {
    ...
    updatePreviewComments(_comments);
    renderComments(_comments, _anchoredIds, _commentRanges);
