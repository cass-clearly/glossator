...
`,QC=null,XC=null;function ZC(e,r){XC=e,QC=r,_6()}function _6(){let e=document.createElement("style");e.textContent=I6,document.head.appendChild(e)}function e0(){let e=QC(),r=document.createElement("div");r.className="hf-overlay";let t=document.createElement("div");t.className="hf-modal";let n=document.createElement("div");n.className="hf-modal-header",n.innerHTML="<h2>Send Feedback to Claude</h2>";let a=document.createElement("button");a.className="hf-modal-close",a.textContent="\xD7",a.addEventListener("click",()=>r.remove()),n.appendChild(a);let o=document.createElement("div");o.className="hf-modal-body";let u=document.createElement("div");if(u.className="hf-modal-footer",t.append(n,o,u),r.appendChild(t),r.addEventListener("click",p=>{p.target===r&&r.remove()}),document.body.appendChild(r),e.length===0){o.innerHTML='<div class="hf-status hf-status-info">No comments yet. Ask reviewers to highlight text and add comments first.</div>';return}let s=document.querySelector(XC.contentSelector||"body").innerHTML,c=JC(s,e);o.innerHTML=`
  ...
  `;let l=document.createElement("button");l.className="hf-btn hf-btn-primary",l.textContent="Copy Prompt",l.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(c),l.textContent="Copied!"}catch{l.textContent="Copy failed"}setTimeout(()=>l.textContent="Copy Prompt",2e3)}),u.appendChild(l);let f=document.createElement("button");f.className="hf-btn hf-btn-secondary",f.textContent="Close",f.addEventListener("click",()=>r.remove()),u.appendChild(f)}function r0(e){let r=[],t=new Map;for(let n of e)n.parent?(t.has(n.parent)||t.set(n.parent,[]),t.get(n.parent).push(n)):r.push(n);return{topLevel:r,repliesByParent:t}}function Qs(e,r){return e.length>r?e.slice(0,r)+"...":e}function t0(e){let r=Date.now()-new Date(e).getTime(),t=Math.floor(r/6e4);if(t<1)return"just now";if(t<60)return`${t}m ago`;let n=Math.floor(t/60);return n<24?`${n}h ago`:`${Math.floor(n/24)}d ago`}var gn=null;function a0(e){gn=document.createElement("div"),gn.className="fb-toast-container",e.appendChild(gn)}function ze(e,r="success"){if(!gn)return;let t=document.createElement("div");t.className=`fb-toast fb-toast-${r}`;let n=document.createElement("span");if(n.textContent=e,t.appendChild(n),r==="error"){let o=document.createElement("button");o.className="fb-toast-dismiss",o.innerHTML="&times;",o.addEventListener("click",()=>n0(t)),t.appendChild(o)}gn.appendChild(t),requestAnimationFrame(()=>{t.classList.add("fb-toast-visible")}),setTimeout(()=>n0(t),r==="error"?8e3:4e3)}function n0(e){e.classList.remove("fb-toast-visible"),e.addEventListener("transitionend",()=>e.remove(),{once:!0}),setTimeout(()=>{e.parentNode&&e.remove()},300)}function i0(e,r){return r<=0?0:(e%r+r)%r}var ri=y(s0());var R6=320,f0="feedback-layer-commenter",B=null,fe=null,xe=null,Xs=null,p0=null,Zs=null,ec=null,rc=null,tc=null,Sn=null,A6=null,nc=null,v0=!1,d0=[],ac=new Set,Pe=-1,qn=null,c0=!1;function N6(){c0||(c0=!0,H6())}function Ve(){return localStorage.getItem(f0)||""}function m0({onSubmit:e,onDelete:r,onResolve:t,onReply:n,onEdit:a,onReaction:o,onColorChange:u,defaultColor:s}){p0=e,Zs=r,ec=t,rc=n,tc=a,Sn=o,A6=u,nc=s||null,N6(),B=document.createElement("div"),B.className="fb-sidebar fb-sidebar-collapsed",B.setAttribute("role","complementary"),B.setAttribute("aria-label","Feedback sidebar"),B.innerHTML=`
  ...
  `;let c=document.createElement("button");c.className="fb-sidebar-tab",c.textContent="Feedback",c.addEventListener("click",()=>vt()),document.body.appendChild(c),document.body.appendChild(B),a0(B),fe=B.querySelector(".fb-comment-list"),fe.setAttribute("role","list"),xe=B.querySelector(".fb-form-section");let l=B.querySelector(".fb-name-input");l.addEventListener("input",()=>{localStorage.setItem(f0,l.value.trim())}),B.querySelector(".fb-ai-btn").addEventListener("click",()=>e0()),B.querySelector(".fb-shortcuts-btn").addEventListener("click",()=>y0()),B.querySelector(".fb-sidebar-toggle").addEventListener("click",()=>ic());let d=B.querySelector(".fb-show-resolved-cb");d.addEventListener("change",()=>{v0=d.checked,Re(d0,ac)}),document.addEventListener("keydown",j6)}function vt(){B.classList.remove("fb-sidebar-collapsed"),document.querySelector(".fb-sidebar-tab").classList.add("fb-sidebar-tab-hidden"),$6(),fe?.querySelector(".fb-thread .fb-cmt-card")&&ti(0)}function ic(){B.classList.add("fb-sidebar-collapsed"),document.querySelector(".fb-sidebar-tab").classList.remove("fb-sidebar-tab-hidden"),b0(),Pe=-1,document.body.focus()}function h0(){let e=document.activeElement;if(!e)return!1;let r=e.tagName;return r==="TEXTAREA"||r==="INPUT"||r==="SELECT"||e.isContentEditable}function xn(){return fe?Array.from(fe.querySelectorAll(".fb-thread > .fb-cmt-card:first-child")):[]}function ti(e){let r=xn();if(r.length===0)return;e=i0(e,r.length),Pe=e,fe.querySelectorAll(".fb-cmt-card").forEach(a=>a.classList.remove("fb-cmt-active"));let t=r[e];t.classList.add("fb-cmt-active"),t.scrollIntoView({behavior:"smooth",block:"nearest"});let n=t.closest(".fb-thread");n?.dataset.commentId&&ac.has(n.dataset.commentId)&&(bn(n.dataset.commentId),Js(n.dataset.commentId))}function M6(e){if(B.classList.contains("fb-sidebar-collapsed"))return;let r=e.key;if(r==="Escape"){e.preventDefault(),ic();return}if(!h0()){if(r==="Enter"){let t=xn();if(Pe>=0&&Pe<t.length){e.preventDefault();let a=t[Pe].closest(".fb-thread")?.querySelector(".fb-reply-btn");a&&a.click()}return}if(r==="j"){if(e.preventDefault(),xn().length===0)return;let n=Pe<0?0:Pe+1;ti(n);return}if(r==="k"){e.preventDefault();let t=xn();if(t.length===0)return;let n=Pe<0?t.length-1:Pe-1;ti(n);return}if(r==="?"){e.preventDefault(),y0();return}}}function y0(){let e=document.querySelector(".fb-shortcuts-overlay");if(e){e.remove();return}let r=document.createElement("div");r.className="fb-shortcuts-overlay";let t=document.createElement("div");t.className="fb-shortcuts-modal",t.innerHTML=`
  ...
  `;let n=()=>r.remove();t.querySelector(".fb-shortcuts-close").addEventListener("click",n),r.addEventListener("click",a=>{a.target===r&&n()}),r.addEventListener("keydown",a=>{a.key==="Escape"&&(a.stopPropagation(),n())}),r.appendChild(t),document.body.appendChild(r),t.querySelector(".fb-shortcuts-close").focus()}function j6(e){h0()||e.key==="s"&&(e.preventDefault(),L6())}function L6(){B.classList.contains("fb-sidebar-collapsed")?vt():ic()}function $6(){b0(),qn=M6,document.addEventListener("keydown",qn)}function b0(){qn&&(document.removeEventListener("keydown",qn),qn=null)}function g0(e){if(vt(),!Ve()){let u=B.querySelector(".fb-name-input");u.classList.add("fb-name-input-error"),setTimeout(()=>u.classList.remove("fb-name-input-error"),2e3)}Xs=e,xe.style.display="";let r=Object.entries(he.COLOR_PRESETS),t=nc||he.DEFAULT_COLOR;xe.innerHTML=`
  ...
  `;let n=t;xe.querySelectorAll(".fb-color-swatch").forEach(u=>{u.addEventListener("click",s=>{s.preventDefault(),xe.querySelectorAll(".fb-color-swatch").forEach(c=>c.classList.remove("fb-color-swatch-active")),u.classList.add("fb-color-swatch-active"),n=u.dataset.color})});let a=xe.querySelector(".fb-form-textarea");a.focus();let o=()=>{if(!Ve()){let s=B.querySelector(".fb-name-input");s.focus(),s.style.outline="2px solid var(--remarq-danger)",setTimeout(()=>s.style.outline="",2e3);return}let u=a.value.trim();u&&(p0({comment:u,commenter:Ve(),color:n}),xe.style.display="none",Xs=null)};xe.querySelector(".fb-submit-btn").addEventListener("click",o),a.addEventListener("keydown",u=>{u.key==="Enter"&&(u.metaKey||u.ctrlKey)&&(u.preventDefault(),o())}),xe.querySelector(".fb-cancel-btn").addEventListener("click",()=>{xe.style.display="none",Xs=null}),xe.scrollIntoView({behavior:"smooth",block:"nearest"})}function Re(e,r=new Set,t=new Map){d0=e,ac=r,Pe=-1,fe.innerHTML="";let{topLevel:n,repliesByParent:a}=r0(e),o=[],u=[];for(let l of n)r.has(l.id)?o.push(l):u.push(l);o.sort((l,f)=>{let p=t.get(l.id),v=t.get(f.id);return!p||!v?0:p.compareBoundaryPoints(Range.START_TO_START,v)});let s=[...o,...u],c=v0?s:s.filter(l=>l.status!=="closed");if(s.length===0){fe.innerHTML='<div class="fb-empty">No comments yet. Select text to add one.</div>';return}if(c.length===0){fe.innerHTML=`<div class="fb-empty">All ${s.length} comment(s) resolved. Check "Show closed" to see them.</div>`;return}for(let l of c){let f=!r.has(l.id),p=document.createElement("div");p.className="fb-thread",p.setAttribute("role","listitem"),p.setAttribute("tabindex","0"),p.dataset.commentId=l.id,p.appendChild(l0(l,!1,f));let v=a.get(l.id)||[];for(let h of v)p.appendChild(l0(h,!0));let d=document.createElement("button");d.className="fb-reply-btn",d.textContent="Reply",d.addEventListener("click",h=>{h.stopPropagation(),G6(l.id,p,d)}),p.appendChild(d),p.addEventListener("focus",()=>{let m=xn().indexOf(p.querySelector(".fb-cmt-card"));m>=0&&ti(m)}),fe.appendChild(p)}}function l0(e,r,t=!1){let n=e.status==="closed",a=document.createElement("div");a.className="fb-cmt-card"+(n?" fb-cmt-closed":"")+(r?" fb-cmt-reply":"")+(t?" fb-cmt-orphaned":""),a.dataset.id=e.id;let o=t&&e.quote?`<div class="fb-cmt-orphaned-quote">Content Deleted: "${Ke(Qs(e.quote,120))}"</div>`:"";a.innerHTML=`
    ${o}
    <div class="fb-cmt-body">${Ke(e.body)}</div>
    <div class="fb-cmt-meta">
      <span class="fb-cmt-author">${Ke(e.author)}</span>
      <span class="fb-cmt-time">${t0(e.created_at)}</span>
