...
Webhooks let you receive real-time notifications when comment events occur. Remarq sends an HTTP POST to your registered URL with a signed payload whenever a subscribed event fires. Delivery is fire-and-forget with automatic retries (exponential backoff, up to 3 attempts).

### Webhook object

| Field        | Type     | Description                                        |
|--------------|----------|----------------------------------------------------|
| `id`         | string   | Unique identifier (`whk_` prefix)                  |
| `object`     | string   | Always `"webhook"`                                 |
| `url`        | string   | The endpoint URL that receives event payloads      |
| `events`     | string[] | List of subscribed event types                     |
| `active`     | boolean  | Whether the webhook is enabled (default: `true`)   |
| `created_at` | string   | ISO 8601 creation timestamp                        |

> **Note:** The `secret` field is used for payload signing but is never returned in API responses.
...
Remarq POSTs a JSON payload to your webhook URL:
...
The request includes an `X-Remarq-Signature` header containing an HMAC-SHA256 hex digest of the request body, signed with your webhook secret.

**Slack and Discord:** If the webhook URL contains `hooks.slack.com` or `discord.com/api/webhooks`, Remarq automatically formats the payload for that platform.
...
Returns all webhooks ordered by creation time (ascending).
...
```bash
curl http://localhost:3333/webhooks
```
...
| `secret` | string   | Yes      | Secret key for HMAC-SHA256 payload signing |
...
| `400`  | `secret is required`                                             | Missing `secret`        |
| `400`  | `events must be a non-empty array`                               | Missing or empty events |
| `400`  | `Invalid events: ... Valid events: comment.created, comment.resolved, comment.deleted` | Unknown event type |
...
| Status | Message              | Condition                 |
|--------|----------------------|---------------------------|
| `404`  | `Webhook not found`  | No webhook with this ID   |

**Example:**

```bash
curl http://localhost:3333/webhooks/whk_9xR2bNp4mLvQ
```
...
Updates a webhook's URL, events, or active status. All fields are optional.
...
| `events` | string[] | New list of event types (must be non-empty)   |
...
|--------|------------------------------------------------------------------|-------------------------|
| `404`  | `Webhook not found`                                              | No webhook with this ID |
| `400`  | `events must be a non-empty array`                               | Empty events array      |
| `400`  | `Invalid events: ... Valid events: comment.created, comment.resolved, comment.deleted` | Unknown event type |
...
```bash
# Disable a webhook
curl -X PATCH http://localhost:3333/webhooks/whk_9xR2bNp4mLvQ \
  -H "Content-Type: application/json" \
  -d '{"active": false}'
...
curl -X PATCH http://localhost:3333/webhooks/whk_9xR2bNp4mLvQ \
  -H "Content-Type: application/json" \
  -d '{"events": ["comment.created", "comment.deleted"]}'
```
...
**Response:** `200 OK` — returns the deleted webhook.
...
```bash
curl -X DELETE http://localhost:3333/webhooks/whk_9xR2bNp4mLvQ
```
...
To verify that a webhook payload is authentic, compute the HMAC-SHA256 of the raw request body using your secret and compare it to the `X-Remarq-Signature` header:
...
app.post("/webhook", (req, res) => {
  ...
  if (!verifySignature("whsec_your_secret_key", body, signature)) {
